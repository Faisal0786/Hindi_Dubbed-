...
class Vikingfile : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        try {
            ...
            // First try the actual Download / /d/ link
            var downloadUrl = document
                .select("a[href*='/d/']")
                .map { it.attr("href") }
                .firstOrNull { it.isNotBlank() }

            // Fallback: find /d/... directly from HTML
            if (downloadUrl.isNullOrBlank()) {
                downloadUrl = Regex(
                    """https?://(?:www\.)?vikingfile\.com/d/[^"'<>\\\s]+"""
                ).find(html)?.value
            }

            // Relative /d/ link fallback
            if (downloadUrl.isNullOrBlank()) {
                downloadUrl = Regex(
                    """["'](/d/[^"'<>\\\s]+)["']"""
            ...
            if (downloadUrl.isNullOrBlank()) {
                Log.w(name, "Download URL not found")
                Log.d(name, "Page URL: $url")
                return
            }
            ...
            val finalUrl = when {
                ...
                downloadUrl.startsWith("/") ->
                    "https://vikingfile.com$downloadUrl"

                else ->
                    "https://vikingfile.com/$downloadUrl"
            }

            Log.d(name, "Resolved download URL: $finalUrl")
            ...
            callback.invoke(
                newExtractorLink(
                    ...
                    this.referer = url
                    this.quality = getIndexQuality(finalUrl)
                }
            )

            Log.d(name, "SUCCESS: ExtractorLink sent")
