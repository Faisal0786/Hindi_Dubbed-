...
data class WpPost(
    ...
    @param:JsonProperty("title") val title: Map<String, Any>?,
    @param:JsonProperty("content") val content: Map<String, Any>?
)
...
class SDMoviesProvider : MainAPI() {
    ...
    override suspend fun getMainPage(
        ...
        val url = if (page <= 1) "$mainUrl$basePath/" else "$mainUrl$basePath/page/$page/"

        Log.d("SDMovies", "Loading category: ${request.name} | URL: $url")

        val document = try {
            app.get(url).document
        } catch (e: Exception) {
            Log.e("SDMovies", "Category load failed: ${e.message}")
            return newHomePageResponse(request.name, emptyList())
        }

        val home = document.select("main#main .site-main h3 a").mapNotNull { titleLink ->
            val postUrl = titleLink.attr("href").trim()
            val title = titleLink.text().trim()

            if (postUrl.isBlank() || title.isBlank()) return@mapNotNull null

            val card = titleLink.parent()?.parent()?.parent()
        ...
        Log.d("SDMovies", "${request.name}: ${home.size} items found")
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        if (isSeries(rawTitle)) {
            ...
            forms.forEachIndexed { index, form ->
                val payloadMap = form.select("input").associate { it.attr("name") to it.attr("value") }
            ...
            Log.d("SDMovies", "Movie forms preserved = ${moviePayloads.size}")
    ...
    override suspend fun loadLinks(
        ...
        val payloads: List<Map<String, String>> = try {
            ...
            if (root.isArray) {
                root.mapNotNull { node ->
                    ...
                    node.fields().asSequence().associate { entry -> entry.key to entry.value.asText() }
                }
            } else if (root.isObject) {
                listOf(root.fields().asSequence().associate { entry -> entry.key to entry.value.asText() })
            } else {
                Log.e("SDMovies", "Invalid payload JSON")
                return false
            }
        } catch (e: Exception) {
            Log.e("SDMovies", "Failed to parse payload: ${e.message}", e)
            return false
        }
        ...
        Log.d("SDMovies", "Total payloads to process = ${payloads.size}")
        var foundLinks = false

        payloads.forEachIndexed { index, payloadMap ->
            Log.d("SDMovies", "PROCESSING FORM #${index + 1} | Payload = $payloadMap")

            val domainPart = payloadMap["id"]?.trim('/')?.takeIf { it.isNotBlank() }
            val filePart = payloadMap["filename"]?.trim('/')?.takeIf { it.isNotBlank() }

            if (domainPart == null || filePart == null) {
                Log.w("SDMovies", "Form #${index + 1}: missing id or filename")
            ...
            Log.d("SDMovies", "Generated Dotflix URL = $dotflixUrl")
            ...
            val dotflixHtml = try {
                ...
                Log.e("SDMovies", "Form #${index + 1}: Dotflix page failed: ${e.message}")
                return@forEachIndexed
            }
            ...
            if (extractedLinks.isEmpty()) {
                Log.w("SDMovies", "Form #${index + 1}: no URLs found")
                return@forEachIndexed
            }
            ...
            for (link in extractedLinks) {
                ...
                if (lowerLink.contains("adsboosters") || lowerLink.contains("yonogames") || lowerLink.contains("w3.org") ||
                    lowerLink.contains("logo.png") || lowerLink.contains("dtflix.ink/logo") || lowerLink.contains("t.me") ||
                    lowerLink.contains("telegram") || lowerLink.contains("googletagmanager.com") || lowerLink.contains("googlesyndication.com") ||
                    lowerLink == "https://dtflix.ink" || lowerLink == "https://dtflix.ink/share"
                ) {
                    continue
                }
                ...
                if (lowerLink.contains(".r2.dev/")) {
                    foundLinks = true
                    Log.d("SDMovies", "R2 [$qualityText] = $link")
                    callback.invoke(
                        newExtractorLink(
                            source = "SDMovies",
                ...
                if (lowerLink.contains("googleusercontent.com")) {
                    foundLinks = true
                    Log.d("SDMovies", "Google CDN [$qualityText]")
                    callback.invoke(
                        newExtractorLink(
                            source = "SDMovies",
                ...
                if (lowerLink.contains("pixeldrain") || lowerLink.contains("vikingfile.com") || lowerLink.contains("transfer.it")) {
                    Log.d("SDMovies", "Extractor [$qualityText] = $link")
                    try {
                        ...
                        Log.e("SDMovies", "Extractor failed: ${e.message}")
                    }
                    continue
                }
            }
        ...
        Log.d("SDMovies", "Extraction finished | foundLinks = $foundLinks")
        return foundLinks
    }
}
