...
class MovieBoxProvider : MainAPI() {
    ...
    private fun generateXTrSignature(method: String, url: String, bodyStr: String?, ts: Long): String {
        ...
        val (bHash, bLen) = if (bBytes != null && bBytes.isNotEmpty()) {
    ...
    private suspend fun ensureSession(): String {
        ...
        for (i in HOST_POOL.indices) {
            ...
            try {
                ...
                val (code, _, respText) = withContext(Dispatchers.IO) {
    ...
    private suspend fun apiRequest(method: String, path: String, bodyStrForSig: String? = null): String {
        ...
        for (i in HOST_POOL.indices) {
            ...
            try {
                ...
                if (method == "POST") {
                    val reqBody = (bodyStrForSig ?: "").toRequestBody("application/json".toMediaTypeOrNull())
                    requestBuilder.post(reqBody)
                } else {
                    requestBuilder.get()
                }

                val (code, xUser, respText) = withContext(Dispatchers.IO) {
                    val response = app.baseClient.newCall(requestBuilder.build()).execute()
                    Triple(response.code, response.header("x-user"), response.body?.string() ?: "")
                }
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        val respText = apiRequest("POST", "/wefeed-mobile-bff/subject-api/search/v2", bodyStr)
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        playInfo?.streams?.forEach { stream ->
            ...
            callback(
                newExtractorLink(
                    source = this.name,
                    name = if (isDash) "Multi-Res ${stream.codecName}" else "${qualities}p ${stream.codecName}",
                    url = manifestUrl,
    ...
    private data class Subject(val id: String?, val subjectId: String?, val title: String?, val name: String?, val subjectType: Int?, val stype: Int?, val releaseDate: String?, val year: String?, val cover: Cover?, val coverUrl: String?, val poster: String?, val description: String?, val intro: String?, val imdbRatingValue: String?, val genres: List<String>?, val genre: List<String>?)
