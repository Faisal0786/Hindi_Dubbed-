...
class MovieBoxProvider : MainAPI() {
    ...
    private fun md5Hex(data: ByteArray): String =
        MessageDigest.getInstance("MD5").digest(data).joinToString("") {
            "%02x".format(it)
        }
    ...
    private fun generateXTrSignature(
        ...
        val (bHash, bLen) = if (bBytes != null) {
            Pair(
                md5Hex(
                    if (bBytes.size > 102400) {
                        bBytes.copyOfRange(0, 102400)
                    } else {
                        bBytes
                    }
                ),
        ...
        val mac = Mac.getInstance("HmacMD5").apply {
            init(SecretKeySpec(SECRET_BYTES, "HmacMD5"))
        }

        val sigB64 = Base64.encodeToString(
            mac.doFinal(canonicalStr.toByteArray()),
            Base64.NO_WRAP
        )
    ...
    private suspend fun ensureSession(): String {
        ...
        if (sessionToken != null && now < sessionExpiry - 60) {
            return sessionToken!!
        }
    ...
    override suspend fun getMainPage(
        ...
        items.forEach { group ->
            ...
            group.banner?.banners?.forEach {
                it.subject?.let { s -> subjects.add(s) }
            }

            group.customData?.items?.forEach {
                it.subject?.let { s -> subjects.add(s) }
            }
            ...
            val searchResponses = subjects.mapNotNull {
                it.toSearchResponse()
            }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        if (internalData.isMovie) {
            ...
            seasonData?.seasons?.forEach { s ->
                ...
                for (eNum in 1..maxEp) {
                    val epDataString =
                        "{\"id\":\"${internalData.id}\",\"isMovie\":false,\"season\":$sNum,\"episode\":$eNum}"
    ...
    override suspend fun loadLinks(
        ...
        playInfo?.streams?.forEach { stream ->
            ...
            if (signCookie.contains("CloudFront-Policy=")) {
                val policyB64 = signCookie
                    .substringAfter("CloudFront-Policy=")
                    .substringBefore(";")
                    .replace("-", "+")
                    .replace("_", "=")
                    .replace("~", "/")
                ...
                try {
                    ...
                    val stmt =
                        (policyJson["Statement"] as? List<Map<String, Any>>)
                            ?.firstOrNull()
            ...
            if (
                manifestUrl.contains("notice.mp4") ||
                manifestUrl.contains("aa348f2541d")
            ) {
                return@forEach
            }

            val isDash =
                manifestUrl.endsWith(".mpd") ||
                    stream.format?.equals("DASH", true) == true

            val qualities =
                stream.resolutions
                    ?.split(",")
                    ?.maxOfOrNull {
                        it.trim().toIntOrNull() ?: 1080
                    }
                    ?: 1080
            ...
            if (signCookie.isNotEmpty()) {
                headers["Cookie"] = signCookie.trimEnd(';')
            }
            ...
            callback(
                newExtractorLink(
                    source = this.name,
                    name = if (isDash) {
                        "Multi-Res ${stream.codecName}"
                    } else {
                        "${qualities}p ${stream.codecName}"
                    },
                    url = manifestUrl,
                    type = if (isDash) {
                    ...
                    quality = getQualityFromName(qualities.toString())
    ...
    private fun Subject.toSearchResponse(): SearchResponse? {
        val isMovie =
            (this.subjectType ?: this.stype ?: 1) == 1
        ...
        val title =
            cleanTitle(this.title ?: this.name ?: "")
        ...
        val internalDataString =
            "{\"id\":\"$id\",\"isMovie\":$isMovie,\"season\":0,\"episode\":0}"
    ...
    private data class TabData(
        ...
        val list: List<GroupItem>?
    )

    private data class GroupItem(
        val name: String?,
        val title: String?,
        val banner: BannerGroup?,
        val customData: CustomDataGroup?,
    ...
    private data class PlayInfoResult(
        val data: PlayData?
    )
