...
private fun parseSubjectToSearchResponse(subject: JSONObject?, seenIds: HashSet<String>): SearchResponse? {  
        ...
        try {
            ...
            if (dataObj != null) {
                ...
                if (resultsArr != null && resultsArr.length() > 0) {
                    itemsArray = resultsArr.optJSONObject(0)?.optJSONArray("subjects")
                }
                if (itemsArray == null) itemsArray = dataObj.optJSONArray("list")
            }
            
            if (itemsArray == null) {
                val resultsArr = rootObj.optJSONArray("results")
                if (resultsArr != null && resultsArr.length() > 0) {
                    itemsArray = resultsArr.optJSONObject(0)?.optJSONArray("subjects")
                }
            }
            
            if (itemsArray == null) itemsArray = rootObj.optJSONArray("list")
            
            if (itemsArray != null) {
                for (i in 0 until itemsArray.length()) {
                    val subject = itemsArray.optJSONObject(i)
                    parseSubjectToSearchResponse(subject, seenIds)?.let { searchResponses.add(it) }
                }
            }
        } catch (e: Exception) {
            throw Exception("Search Parsing Failed: ${e.message}\nJSON: ${respText.take(500)}")
        }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val poster = subjectObj.optJSONObject("cover")?.optString("url")?.takeIf { it.isNotBlank() }
            ...
            ?: subjectObj.optString("poster").takeIf { it.isNotBlank() }
        ...
        if (rawDuration != null) {
            if (rawDuration is Number) {
                durationMinutes = (rawDuration.toLong() / 60L).toInt().takeIf { it > 0 }
            } else if (rawDuration is String) {
                val trimmed = rawDuration.trim()
                if (trimmed.endsWith("m", ignoreCase = true)) {
                    durationMinutes = trimmed.dropLast(1).toIntOrNull()
                } else {
                    durationMinutes = trimmed.toLongOrNull()?.let { (it / 60L).toInt() }?.takeIf { it > 0 }
                }
            }
        }
        ...
        runCatching {
            val sRoot = JSONObject(seasonRespText)
            val seasonsArr = sRoot.optJSONObject("data")?.optJSONArray("seasons") 
                ?: sRoot.optJSONArray("seasons")
