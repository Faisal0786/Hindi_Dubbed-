...
class Net77Provider : MainAPI() {
    ...
    data class Net77SearchResponse(
    ...
    override suspend fun getMainPage(
        ...
        document.select("div.lolomoRow").forEach { row ->
            ...
            val categoryName =
                row.selectFirst("div.row-header-title")?.text()
                    ?: "Trending"
            ...
            row.select("div.title-card-container").forEach { card ->
                ...
                val title =
                    card.selectFirst("a[aria-label]")
                        ?.attr("aria-label")
                        ?: "Unknown"

                val imgNode =
                    card.selectFirst("img.boxart-image")
                ...
                list.add(
                    newMovieSearchResponse(
                        name = title,
                        url = ContentData(id, title).toJson(),
                        type = TvType.Movie
                    ) {
    ...
    override suspend fun search(
        ...
        val url =
            "$mainUrl/search.php?s=$query&t=$currentTime"
        ...
        val parsed =
            parseJson<Net77SearchResponse>(response)
        ...
        return parsed.searchResult?.mapNotNull { item ->
            ...
            newMovieSearchResponse(
                ...
                this.posterUrl =
                    "https://imgcdn.kim/poster/1920/$id.jpg"
            }
    ...
    override suspend fun load(
        ...
        val data =
            parseJson<ContentData>(url)

        val currentTime =
            (System.currentTimeMillis() / 1000).toString()

        val postUrl =
            "$mainUrl/post.php?id=${data.id}&t=$currentTime"
        ...
        val details =
            parseJson<DetailResponse>(response)

        val title =
            details.title ?: data.title

        val poster =
            "https://imgcdn.kim/poster/1920/${data.id}.jpg"
    ...
    override suspend fun loadLinks(
        ...
        val playlistHeaders =
            mapOf(
                "Referer" to
                    "https://net52.cc/play.php?id=$contentId&in=$cleanHash"
            )
        ...
        val parsedPlaylist =
            parseJson<List<PlaylistResponse>>(
                playlistJson
            ).firstOrNull()
                ?: return false
